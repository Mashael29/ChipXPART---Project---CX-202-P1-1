...
void load_program(const char *InputFile, char program[], int *program_size) {
    ...
    if (MyFile == NULL) { // If the file did not open, print this message
...
int execute_program(char *program, int program_size) {
	...
	printf("Instruction %d: RA=%d \n",Program_Counter,RA);
	printf("Instruction %d: RB=%d \n",Program_Counter,RB);
        ...
        if (J && (C == 0 || carry)) {
           Program_Counter = instruction & 0x0F; // Jump to specific instruction
